...
import useIsCommunication from '../hook/useIsCommunication';
import Image from 'next/image';

export default function Nav() {
  const [path, setPath] = useState<BottomNav>('home');
  const pathArr = usePathname().split('/');
  const isCommunication = useIsCommunication();
  ...
  return (
    <nav className="fixed bottom-0 flex h-14 w-full max-w-[450px] justify-center bg-white text-xs">
      ...
      {isCommunication && (
        <Link
          href={''}
          className="absolute right-[25px] top-[-74px] flex h-14 w-14 items-center justify-center rounded-[50px] bg-blue-0 hover:bg-blue-4">
          <Image
            src={'/icon_writeapost.svg'}
            width={40}
            height={24}
            alt="writeapost"
          />
